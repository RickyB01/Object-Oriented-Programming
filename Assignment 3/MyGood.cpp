...
namespace aid {
	...
	std::fstream& Test::load(std::fstream& file) {
		...
		file.clear();
		file.close();
		product = MyGood(GoodSku, GoodName, GoodUnits, quantity, taxed_ != 0, price_, qtyNeeded);
		return file;
	}
	...
	std::istream& Test::read(std::istream& is) {
		...
		is >> qtyNeeded;
		product = MyGood(GoddSku, GoodName, GoodUnits, quantity, taxed_ != 0, price_, qtyNeeded);
		return is;
	}
